 ...
 */
export default async function fetchAppointments({ memberId, onInit, onFinish, onError, onFinally }) {
  try {
    if (typeof onInit === 'function') onInit() // Execute 'onInit' callback
    const result = await apiProfile.get({ url: `/appointments?memberId=${memberId}` })
