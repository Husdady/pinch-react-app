 ...
 */
export default async function fetchProperties({ clientId, onInit, onFinish, onError, onFinally }) {
  try {
    if (typeof onInit === 'function') onInit() // Execute 'onInit' callback
    const result = await apiProfile.get({ url: `/properties?clientId=${clientId}` })
