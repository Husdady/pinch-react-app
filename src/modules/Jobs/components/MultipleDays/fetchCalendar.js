 ...
 */
export default async function fetchCalendar({ memberId, onInit, onFinish, onError, onFinally }) {
  try {
    if (typeof onInit === 'function') onInit() // Execute 'onInit' callback
    const result = await apiProfile.get({ url: `/schedule?memberId=${memberId}` })
